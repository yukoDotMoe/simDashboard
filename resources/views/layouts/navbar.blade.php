                    ...
                    @if($link['active'])
                        @if(!$link['dropdown'])
                            <li class="relative px-6 py-3">
                                ...
                                @endif
                                <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 @if(request()->segment(1) == $link['redirect']) text-gray-800 @else hover:text-gray-800 @endif dark:hover:text-gray-200" href="{{ ($link['singleLink']) ? $link['redirect'] : route($link['redirect']) }}">
                                    {!!
                                        str_replace([
                                            'stroke-width="1.5"',
                                            'svg'
                                    ...
                                    <ul x-transition:enter="transition-all ease-in-out duration-300" x-transition:enter-start="opacity-25 max-h-0" x-transition:enter-end="opacity-100 max-h-xl" x-transition:leave="transition-all ease-in-out duration-300" x-transition:leave-start="opacity-100 max-h-xl" x-transition:leave-end="opacity-0 max-h-0" class="p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-gray-50 dark:text-gray-400 dark:bg-gray-900" aria-label="submenu">
                                        @foreach($link['dropdown'] as $dropdown)
                                            @if(request()->segment(2) == $dropdown['link'])
                                                <span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg" aria-hidden="true"></span>
                                            @endif
                                            <li class="px-2 py-1 transition-colors duration-150 @if(request()->segment(1) == $link['redirect']) text-gray-800 @else hover:text-gray-800 @endif dark:hover:text-gray-200">
                    ...
                    @if($link['active'])
                        <li class="relative px-6 py-3">
                            @if(request()->segment(2) == $nav)
                                <span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg" aria-hidden="true"></span>
                            @endif
                            <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 @if(request()->segment(2) == $nav) text-gray-800 @else hover:text-gray-800 @endif dark:hover:text-gray-200" href="{{ ($link['singleLink']) ? $link['redirect'] : route($link['redirect']) }}">
                                {!!
                                    str_replace([
                                        'stroke-width="1.5"',
                                        'svg'
