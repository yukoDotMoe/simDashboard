...
@section('content')
    <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200"> Quản Lý Nhà Mạng </h2>
        ...
        >
            ...
            <div class="mt-4 mb-6">
                ...
                <p class="mb-2 text-lg font-semibold text-gray-700 dark:text-gray-300" >
                    Đang chỉnh sửa nhà mạng: <span id="serviceName"></span> <small class="text-gray-400">(ID: <span id="serviceId"></span>)</small>
                </p>
                <!-- Modal description -->
                <form>
    ...
    <div class="my-6 justify-center flex-1 lg:mr-32">
        ...
        <div class="mt-4">
            <button onclick="location.href='{{ route('admin.createNetwork') }}';"
                    class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
            >
                <span>Tạo Nhà Mạng</span>
        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                    ...
                    <th class="px-4 py-3">Tên Nhà Mạng</th>
                    <th class="px-4 py-3">Số lượng SĐT</th>
                ...
                @foreach($data['networks'] as $task)
                    <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-4 py-3 text-sm">{{ $task['uniqueId'] }}</td>
                        <td class="px-4 py-3 text-sm">{{ $task['networkName'] }}</td>
                        <td class="px-4 py-3 text-sm">{{ $data['simList'][$task['id']] }}</td>
    ...
    <script>
        $( document ).ready(function() {
            function fillToModal(data) {
                ...
                $('#serviceName').text(data.networkName);
            ...
            $('#serviceBtn').click(function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: `{{ route('admin.networkEdit') }}?id=${simId}&status=${status}`,
                    cache: false,
                    success: function (data) {
                        ...
                        vt.success("Đã lưu thông tin nhà mạng", {
                            title: "Thành công",
                            position: "top-right",
                        })
                        location.reload();
            ...
            $('.delete').click(function (e) {
                e.preventDefault();
                const simId = $(this).attr('data-sim');
                var x = window.confirm("Bạn có chắc muốn xoá nhà mạng này?");
                if(!x) return false;
                $.ajax({
                    type: "POST",
                    url: `{{ route('admin.networkEdit') }}?id=${simId}&delete=1`,
                    cache: false,
                    success: function (data) {
                        console.log(data)
                        if(data.status > 200)
            ...
            $('.editBtn').click(function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.networks') }}/"+simId,
                    cache: false,
                    success: function (data) {
                        fillToModal(data)
                    },
